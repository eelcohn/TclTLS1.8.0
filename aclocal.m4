...
AC_DEFUN([TCLTLS_SSL_OPENSSL_CHECK_PROTO_VER], [
	...
	if test "$openssl_version" = "1.1.0"; then
		AC_MSG_CHECKING([if $2 protocol is available])
		if test "[$]$1" = "true" -o "[$]$1" = "force"; then
			# Translate version string to OpenSSL post-1.1.0 style pre-processor definitions
			AS_CASE([$2],
				# The SSL2 protocol is not available in OpenSSL 1.0.2g and higher, and the #define OPENSSL_NO_SSL2 does not exist.
				# SSL2_VERSION is used to signal that SSL2 is not available on OpenSSL 1.1.0 and higher
				[SSLv2], [openssl_no_define="SSL2_VERSION"],
				[SSLv3], [openssl_no_define="OPENSSL_NO_SSL3"],
				[TLSv1.0], [openssl_no_define="OPENSSL_NO_TLS1"],
				[TLSv1.1], [openssl_no_define="OPENSSL_NO_TLS1_1"],
				[TLSv1.2], [openssl_no_define="OPENSSL_NO_TLS1_2"],
				[TLSv1.3], [openssl_no_define="OPENSSL_NO_TLS1_3"]
			)
			AC_LINK_IFELSE(
				[AC_LANG_PROGRAM(
					[
						...
						#ifdef $openssl_no_define
						#error Not supported
						#endif
					])],
				[AC_MSG_RESULT(yes); $1=yes],
				[AC_MSG_RESULT(no); $1=no
					AC_DEFINE([$3], 1, [Define to 1 to enable $2.])
				]
			)
		else
		...
		# Translate version string to OpenSSL pre-1.1.0 style methods
		AS_CASE([$2],
			[SSLv2], [openssl_method="SSLv2_method"],
			[SSLv3], [openssl_method="SSLv3_method"],
			[TLSv1.0], [openssl_method="TLSv1_method"],
			[TLSv1.1], [openssl_method="TLSv1_1_method"],
			[TLSv1.2], [openssl_method="TLSv1_2_method"],
			# The TLSv1_3_method does not exist, so AC_CHECK_FUNC will fail, and therefore will signal that TLS1.3 is not available on OpenSSL 1.1.0 and lower
			[TLSv1.3], [openssl_method="TLSv1_3_method"]
		)

		if test "[$]$1" = "true" -o "[$]$1" = "force"; then
#			AC_CHECK_LIB([ssl], [$openssl_method], [], [])
			AC_CHECK_FUNC([$openssl_method],
				[] ,
				[
					if test "[$]$1" = "force"; then
						AC_MSG_ERROR([Can't enable $2, method not available.])
					fi
			...
			AC_MSG_CHECKING([for $openssl_method])
			AC_MSG_RESULT([disabled])
		fi

		if test "[$]$1" = "false"; then
			AC_DEFINE($3, [1], [Define this to disable $2 in OpenSSL support])
		fi
	fi
])
...
AC_DEFUN([TCLTLS_SSL_OPENSSL], [
	...
	dnl Check if the OpenSSL libraries can be found
	AC_CHECK_LIB([crypto], [X509_digest], , [AC_MSG_ERROR([Cannot find -lcrypto])])
	AC_CHECK_LIB([ssl], [SSL_accept], , [AC_MSG_ERROR([Cannot find -lssl])])

# Get OpenSSL version; TODO: get real OpenSSL version, not just 1.0.2 or 1.1.0
# Don't use the command-line like cat `openssl version`, this will break when cross-
# compiling, or when compiling against a version of OpenSSL which differs from
# the one installed on your system
	dnl Check which OpenSSL version is used
	AC_CHECK_LIB([ssl], [DH_set0_pqg], [openssl_version="1.1.0"], [openssl_version="1.0.2"])
	AC_MSG_CHECKING([for OpenSSL version])
	if test "$openssl_version" = "1.1.0"; then
		AC_MSG_RESULT([$openssl_version or higher])
	else
		AC_MSG_RESULT([$openssl_version or lower])
	fi

	# And then check if we can compile a basic OpenSSL program
...
# include <openssl/conf.h>
  ...
  SSL_load_error_strings();
	...
	TCLTLS_SSL_OPENSSL_CHECK_PROTO_VER([tcltls_ssl_ssl2], [SSLv2], [NO_SSL2])
	TCLTLS_SSL_OPENSSL_CHECK_PROTO_VER([tcltls_ssl_ssl3], [SSLv3], [NO_SSL3])
	TCLTLS_SSL_OPENSSL_CHECK_PROTO_VER([tcltls_ssl_tls1_0], [TLSv1.0], [NO_TLS1])
	TCLTLS_SSL_OPENSSL_CHECK_PROTO_VER([tcltls_ssl_tls1_1], [TLSv1.1], [NO_TLS1_1])
	TCLTLS_SSL_OPENSSL_CHECK_PROTO_VER([tcltls_ssl_tls1_2], [TLSv1.2], [NO_TLS1_2])
	TCLTLS_SSL_OPENSSL_CHECK_PROTO_VER([tcltls_ssl_tls1_3], [TLSv1.3], [NO_TLS1_3])
