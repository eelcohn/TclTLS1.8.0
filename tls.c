...
static SSL_CTX *CTX_Init(State *statePtr, int proto, char *key,
			...
			char *DHparams);
#else
static SSL_CTX *CTX_Init(State *statePtr, int minproto, int maxproto, char *key,
			char *cert, char *CAdir, char *CAfile, char *ciphers,
			char *DHparams);
#endif
    ...
    Tcl_Obj *CONST objv[];
{
...
#if defined(NO_TLS1) && defined(NO_TLS1_1) && defined(NO_TLS1_2) && defined(NO_TLS1_3) && defined(NO_SSL3) && !defined(NO_SSL2)
    ssl2 = 1;
#endif
#if defined(NO_TLS1) && defined(NO_TLS1_1) && defined(NO_TLS1_2) && defined(NO_TLS1_3) && defined(NO_SSL2) && !defined(NO_SSL3)
    ssl3 = 1;
#endif

#else
	Tcl_Obj *minprotostr	= NULL;
	Tcl_Obj *maxprotostr	= NULL;
	int minproto = 0, maxproto = 0;
	int ssl2 = 0, ssl3 = 0;
	int tls1 = 0, tls1_1 = 0, tls1_2 = 0, tls1_3 = 0;
#endif
	...
	if (request)
		verify |= SSL_VERIFY_CLIENT_ONCE | SSL_VERIFY_PEER;
	if (request && require)
	...
	OPTOBJ( "-tls-version-min", minprotostr);
	OPTOBJ( "-tls-version-max", maxprotostr);

	OPTBAD( "option", "-cadir, -cafile, -certfile, -cipher, -command, -dhparams, -keyfile, -model, -password, -require, -request, -server, -servername, -ssl3, -tls1, -tls1.1, -tls1.2, -tls1.3, -tls-version-min or -tls-version-max");
	return TCL_ERROR;
	}
	if (request)
		verify |= SSL_VERIFY_CLIENT_ONCE | SSL_VERIFY_PEER;
	...
	if (ssl3) {
#if defined(NO_SSL3)
		Tcl_AppendResult(interp, "protocol not available",
			REASON(),
			(char *) NULL);
		return TCL_ERROR;
#else
		if ((tls1) || (tls1_1) || (tls1_2) || (tls1_3)) {
			Tcl_AppendResult(interp, "no multiple protocols allowed",
				REASON(),
				(char *) NULL);
			return TCL_ERROR;
		}
		minproto = SSL3_VERSION;
		maxproto = SSL3_VERSION;
#endif
	}
	if (tls1) {
#if defined(NO_TLS1)
		...
		return TCL_ERROR;
#else
		if ((ssl3) || (tls1_1) || (tls1_2) || (tls1_3)) {
			Tcl_AppendResult(interp, "no multiple protocols allowed",
				REASON(),
				(char *) NULL);
			return TCL_ERROR;
		}
		minproto = TLS1_VERSION;
		maxproto = TLS1_VERSION;
#endif
	}
	if (tls1_1) {
#if defined(NO_TLS1_1)
		...
		return TCL_ERROR;
#else
		if ((ssl3) || (tls1) || (tls1_2) || (tls1_3)) {
			Tcl_AppendResult(interp, "no multiple protocols allowed",
				REASON(),
				(char *) NULL);
			return TCL_ERROR;
		}
		minproto = TLS1_1_VERSION;
		maxproto = TLS1_1_VERSION;
#endif
	}
	if (tls1_2) {
#if defined(NO_TLS1_2)
		...
		return TCL_ERROR;
#else
		if ((ssl3) || (tls1) || (tls1_1) || (tls1_3)) {
			Tcl_AppendResult(interp, "no multiple protocols allowed",
				REASON(),
				(char *) NULL);
			return TCL_ERROR;
		}
		minproto = TLS1_2_VERSION;
		maxproto = TLS1_2_VERSION;
#endif
	}
	if (tls1_3) {
#if defined(NO_TLS1_3)
		...
		return TCL_ERROR;
#else
		if ((ssl3) || (tls1) || (tls1_1) || (tls1_2)) {
			Tcl_AppendResult(interp, "no multiple protocols allowed",
				REASON(),
				(char *) NULL);
			return TCL_ERROR;
		}
		minproto = TLS1_3_VERSION;
		maxproto = TLS1_3_VERSION;
#endif
	}

	/* minimum protocol version */
	if (minprotostr) {
		if (Tcl_GetIndexFromObj(interp, minprotostr, protocols, "protocol", 0, &minproto) != TCL_OK) {
			return TCL_ERROR;
		}
		Tcl_IncrRefCount(minprotostr);
	}

	/* maximum protocol version */
	if (maxprotostr) {
		if (Tcl_GetIndexFromObj(interp, maxprotostr, protocols, "protocol", 0, &maxproto) != TCL_OK) {
			return TCL_ERROR;
		}
		Tcl_IncrRefCount(maxprotostr);
	}
#endif
	...
	if ((ctx = CTX_Init(statePtr, proto, key, cert, CAdir, CAfile, ciphers,
		DHparams)) == (SSL_CTX*)0) {
#else
	if ((ctx = CTX_Init(statePtr, minproto, maxproto, key, cert, CAdir, CAfile, ciphers,
		DHparams)) == (SSL_CTX*)0) {
#endif
	    Tls_Free((char *) statePtr);
	    return TCL_ERROR;
...
static SSL_CTX *
CTX_Init(statePtr, minproto, maxproto, key, cert, CAdir, CAfile, ciphers, DHparams)
    State *statePtr;
    int minproto;
    int maxproto;
    char *key;
    ...
    char *DHparams;
{
    ...
    if (!minproto) {
	Tcl_AppendResult(interp, "minproto not valid", NULL);
	return (SSL_CTX *)0;
    }

    if (!maxproto) {
	Tcl_AppendResult(interp, "maxproto not valid", NULL);
	return (SSL_CTX *)0;
    }

    /* create SSL context */
#if defined(NO_SSL3)
    if (ENABLED(minproto, TLS_PROTO_SSL3)) {
	Tcl_AppendResult(interp, "min protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
    if (ENABLED(maxproto, TLS_PROTO_SSL3)) {
	Tcl_AppendResult(interp, "max protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
#endif
#if defined(NO_TLS1)
    if (ENABLED(minproto, TLS_PROTO_TLS1)) {
	Tcl_AppendResult(interp, "min protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
    if (ENABLED(maxproto, TLS_PROTO_TLS1)) {
	Tcl_AppendResult(interp, "max protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
#endif
#if defined(NO_TLS1_1)
    if (ENABLED(minproto, TLS_PROTO_TLS1_1)) {
	Tcl_AppendResult(interp, "min protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
    if (ENABLED(maxproto, TLS_PROTO_TLS1_1)) {
	Tcl_AppendResult(interp, "max protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
#endif
#if defined(NO_TLS1_2)
    if (ENABLED(minproto, TLS_PROTO_TLS1_2)) {
	Tcl_AppendResult(interp, "min protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
    if (ENABLED(maxproto, TLS_PROTO_TLS1_2)) {
	Tcl_AppendResult(interp, "max protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
#endif
#if defined(NO_TLS1_3)
    if (ENABLED(minproto, TLS_PROTO_TLS1_3)) {
	Tcl_AppendResult(interp, "min protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
    if (ENABLED(maxproto, TLS_PROTO_TLS1_3)) {
	Tcl_AppendResult(interp, "max protocol not supported", NULL);
	return (SSL_CTX *)0;
    }
#endif
    ...
    ctx = SSL_CTX_new (TLS_method());
    if (SSL_CTX_set_min_proto_version(ctx, minproto) != 1) {
	Tcl_AppendResult(interp,
	   "Unable to set minimum supported protocol version",
           (char *) NULL);
	SSL_CTX_free(ctx);
	return (SSL_CTX *)0;
    }
    if (SSL_CTX_set_max_proto_version(ctx, maxproto) != 1) {
	Tcl_AppendResult(interp,
	   "Unable to set maximum supported protocol version",
           (char *) NULL);
	SSL_CTX_free(ctx);
	return (SSL_CTX *)0;
    }
#endif /* #if (OPENSSL_VERSION_NUMBER < 0x10100000L)  || LIBRESSL_VERSION_NUMBER */
    ...
    Tcl_Obj	*CONST objv[];
{
	...
	case C_REQ: {
		...
		if((RSA_generate_key_ex(rsa, keysize, e, NULL)) == 0) {
			Tcl_SetResult(interp,"Error generating private key", NULL);
			EVP_PKEY_free(pkey);
			return TCL_ERROR;
		}
		...
		if (!EVP_PKEY_assign_RSA(pkey, rsa)) {
			Tcl_SetResult(interp,"Error assigning private key", NULL);
			EVP_PKEY_free(pkey);
			return TCL_ERROR;
		}

		out=BIO_new(BIO_s_file());
		if (!BIO_write_filename(out,keyout)) {
			Tcl_SetResult(interp, "Error assigning key output file to bio", NULL);
			EVP_PKEY_free(pkey);
			return TCL_ERROR;
		}
		...
		out=BIO_new(BIO_s_file());
		if (!BIO_write_filename(out,pemout)) {
			Tcl_SetResult(interp, "Error assigning pem output file to bio", NULL);
			EVP_PKEY_free(pkey);
			return TCL_ERROR;
		}
